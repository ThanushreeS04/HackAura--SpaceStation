...
import React, { useState, useEffect, useRef } from 'react';
import './App.css';
import { STATION_INVENTORY } from './stationData'; 
...
function App() {
    ...
    // Function to fetch live YOLO data from Flask API
    const fetchObjectLocation = async (objectName) => {
        // objectName is now guaranteed to be PascalCase (e.g., 'NitrogenTank')
        const queryName = objectName; 
        
        try {
            const response = await fetch(`${API_BASE_URL}/query?object=${encodeURIComponent(queryName)}`);
            
            const data = await response.json();
            
            if (response.ok) {
                // SUCCESS: Check that the keys 'x', 'y', and 'confidence' exist inside data.position
                if (data.position && data.position.x !== undefined) {
                     ...
                     // The 200 response didn't contain valid coordinate data
                     return { found: false, message: "Live data corrupted (invalid coordinates)." };
                }
               
            } else {
                // 404 NOT DETECTED response from Flask
                return { 
                    found: false, 
                    message: data.error || `Object detection failed: ${objectName}` 
                };
            }
        } catch (error) {
            // Network connection failure (CORS/Server Offline)
            console.error("API Communication Error:", error);
            return { 
                found: false, 
                message: "Station communication link offline. Cannot reach live sensor data." 
            };
        }
    };


    // Process voice commands and generate responses - NOW ASYNC
    const processCommand = React.useCallback(async (command) => { // ⚠️ MUST BE ASYNC
        const cmd = command.toLowerCase().trim();
        let response = '';
        
        // --- 🌟 LIVE OBJECT DETECTION LOGIC 🌟 ---
        const objectList = Object.keys(STATION_INVENTORY);
        const locationIntent = cmd.includes('where is') || cmd.includes('locate') || cmd.includes('find') || cmd.includes('show me');

        let objectName = null;
        
        // Find the object key (e.g., 'NitrogenTank') by matching keywords
        for (const key of objectList) {
            if (STATION_INVENTORY[key].keywords.some(keyword => cmd.includes(keyword))) {
                objectName = key; 
                break;
            }
        }

        if (objectName && locationIntent) {
            // 1. Provide immediate feedback while fetching data
            setAssistantResponse(`Searching for ${STATION_INVENTORY[objectName].display_name} via live vision system...`);

            const yoloResult = await fetchObjectLocation(objectName);
            const staticInfo = STATION_INVENTORY[objectName];

            if (yoloResult.found) {
                // --- SUCCESS: Use LIVE data + static procedure ---
                const { x, y, confidence } = yoloResult.location;
                const confPct = Math.round(confidence * 100);
                
                response = `Live detection confirms the **${staticInfo.display_name}** is visible with ${confPct}% confidence. 
                            Its current position is **X:${x.toFixed(1)}, Y:${y.toFixed(1)}** relative to the camera frame. 
                            The standard storage location is **${staticInfo.location}**. 
                            Would you like to initiate the ${staticInfo.access_procedure[0]}?`;
                
                // Trigger navigation
                setCurrentPage(staticInfo.menu_link.toLowerCase()); 
                
            } else {
                // --- FALLBACK: Use static location + error message ---
                response = `Unable to get live coordinates. ${staticInfo.display_name} is not visible, OR the data link is down. 
                            The standard storage location is **${staticInfo.location}**. 
                            ${yoloResult.message} `;
            }
            return response;
        ...
        // -----------------------------------------------------------------
        // --- STATIC COMMANDS (Your existing logic) ---
        // -----------------------------------------------------------------
        ...
        // ... (All other static commands you already wrote) ...
        
        // Hardcoded safety checks based on dashboard parameters (for variety)
        else if (cmd.includes('low oxygen') || cmd.includes('oxygen alarm')) {
             response = 'WARNING: Oxygen levels are at 98.5 percent. This is the last safe threshold. Initiating Protocol O2-CHECK-002: Check O2 valve status immediately!';
        }
        else if (cmd.includes('power issue') || cmd.includes('energy fault')) {
            response = 'Power is currently 100 percent. All solar collection is nominal. If you observe any power drops, initiate Manual Power Transfer Protocol P-XFR-003.';
        }
        
        // Default (Keep this at the very end!)
        else {
            response = 'I did not understand that command. Try saying help to see all available commands, or ask me about systems, oxygen, power, temperature, navigation, or anything else about the station!';
        }
        ...
        if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
            ...
            recognition.onresult = (event) => {
                ...
                if (finalTranscript) {
                    ...
                    // Set a timer to process after 2 seconds of silence
                    silenceTimerRef.current = setTimeout(async () => { // ⚠️ ASYNC
                        // Process command and get response
                        const response = await processCommand(finalTranscript); // ⚠️ AWAIT
                        setAssistantResponse(response);
    ...
    // Manual command trigger (for testing or button clicks)
    const triggerCommand = async (commandText) => { // ⚠️ ASYNC
        setTranscript(commandText);
        const response = await processCommand(commandText); // ⚠️ AWAIT
        setAssistantResponse(response);
        ...
        <div className="min-h-screen bg-space-blue flex">
            ...
            <div className="flex-1 bg-space-blue relative">
                ...
                <div className="p-8">
                    ...
                    {/* ... (Your other page content like System, Tasks, Procedures, Assessment) ... */}
                    ...
                    {(transcript || assistantResponse) && (
                        <div className="bg-gray-800 text-white p-4 rounded-lg max-w-md shadow-xl border border-gray-700">
                            ...
                            {assistantResponse && (
                                <div className={transcript ? 'mt-3 pt-3 border-t border-gray-700' : ''}>
                                    <p className="text-xs text-gray-400 mb-1">Assistant:</p>
                                    <p className="text-sm" dangerouslySetInnerHTML={{ __html: assistantResponse }}></p>
                                </div>
                            )}
                        </div>
                    )}
